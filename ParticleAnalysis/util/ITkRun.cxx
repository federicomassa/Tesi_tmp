...
int main( int argc, char* argv[] ) {
  ...
  int idTarget          = atoi(argv[3]);
  int EnergyTarget      = atoi(argv[4]);
  // int SkipEvent = atoi(argv[5]);
  ...
  std::cout << "Target particle E  : " << EnergyTarget << " [MeV]" << std::endl;
  // std::cout << "Skip event         : " << SkipEvent << std::endl;
  ...
  // SH::DiskListLocal list(inDir.c_str());
  // need this if you want to run on a single file!
  // SH::scanDir (sh, list, "group.det-indet.4246665.ID1._000056.pool.root");
   // SH::scanDir (sh, list, "testDxAOD.pool.root");
  // SH::scanDir (sh, list, "testpi50300DxAOD.pool.root");
  ...
  ITkStudy *alg = new ITkStudy;
  alg -> SetTargetParticleID(idTarget);
  alg -> SetTargetParticleE(EnergyTarget);
  // alg -> SetSkipEvent(SkipEvent);
  job.algsAdd( alg );
